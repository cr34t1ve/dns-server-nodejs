...
function processRr(
  ...
  let isMx = Number(splittedLine[totalLength - 1]);
  //    IN      MX      10            mail.example.com
  //    class   type    preference    data
  //                    isMx
  ...
  switch (totalLength) {
    case 5:
      for (let index = 0; index < totalLength; index++) {
        ...
        if (!element.includes(".")) {
          // to exlude processing IP Addresses
          if (parseInt(element)) {
            if (!isMx) {
              // extract the TTL value and remove it from the items array
              containsTtl = true;
              previousTtl = element;
