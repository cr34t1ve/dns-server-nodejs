...
server.on("message", async (msg, rinfo) => {
  let TID = msg.slice(0, 2); // Transaction ID
...
function getFlags(flags) {
  ...
  let OPCode = bitsExtract(byte1); // extracts bits 1 to 5 from the OPCode
  let AA = "1";
  let TC = "0";
  let RD = "0";
...
async function getRecords(data) {
  ...
  //   console.log("get domain result:", result);
...
function getDomain(data) {
  ...
  let recordType = data.slice(y, y + 2); // after terminator, exists record type which is 2 bytes
...
function recordToBytes(recordType, record) {
  ...
  if (recordType == "A") {
    ...
    let serial = domainToHex(record["serial"]);
    let refresh = domainToHex(record["refresh"]);
    let retry = domainToHex(record["retry"]);
    let expire = domainToHex(record["expire"]);
    let minimum = domainToHex(record["minimum"]);
